...
type PlayerStats struct {
	...
	dmgJustTaken, healJustTaken       string
}
...
func (p *PlayerStats) Draw(screen *ebiten.Image, font font.Face) {
	...
	if p.healJustTaken != "" {
		text.Draw(screen, p.healJustTaken, font, p.infoX, p.infoY+font.Metrics().Height.Ceil(), p.hpCol)
	}
	if p.dmgJustTaken != "" {
		text.Draw(screen, p.dmgJustTaken, font, p.infoX, p.infoY+font.Metrics().Height.Ceil(), p.dmgCol)
	}
